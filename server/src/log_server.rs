...
pub fn event_user_subscribed(team_uuid: &str, user_uuid: &str) -> c_int {
    unsafe { server_event_user_subscribed(cstr(team_uuid).as_ptr(), cstr(user_uuid).as_ptr()) }
}

pub fn event_user_unsubscribed(team_uuid: &str, user_uuid: &str) -> c_int {
    unsafe { server_event_user_unsubscribed(cstr(team_uuid).as_ptr(), cstr(user_uuid).as_ptr()) }
}

pub fn event_user_created(user_uuid: &str, user_name: &str) -> c_int {
    unsafe { server_event_user_created(cstr(user_uuid).as_ptr(), cstr(user_name).as_ptr()) }
}

pub fn event_user_loaded(user_uuid: &str, user_name: &str) -> c_int {
    unsafe { server_event_user_loaded(cstr(user_uuid).as_ptr(), cstr(user_name).as_ptr()) }
}
...
pub fn event_private_message_sent(
    sender_uuid: &str,
    receiver_uuid: &str,
    message_body: &str,
) -> c_int {
